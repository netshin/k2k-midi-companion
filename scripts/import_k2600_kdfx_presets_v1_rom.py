#!/usr/bin/env python3

from __future__ import annotations

import json
import re
import subprocess
from pathlib import Path


ROOT = Path(__file__).resolve().parents[1]
LOOKUP_PATH = ROOT / "Kurzweil" / "K2600" / "k2600_kdfx_lookup.json"
SOURCE_PATH = Path(
    "/Volumes/Logic Library/Users/shayrak/Sync/Documents/Code/K2600-Midi-Companion/"
    "supporting materials/K2600/Extracted/KDFX Presets/K2600 KDFX Presets - v1 ROM.pdf"
)


def normalize_name(name: str) -> str:
    return re.sub(r"[^a-z0-9]+", "", name.lower())


def add_preset_index(preset_index: dict[str, int | list[int]], normalized_name: str, preset_id: int) -> None:
    existing = preset_index.get(normalized_name)
    if existing is None:
        preset_index[normalized_name] = [preset_id]
        return
    if isinstance(existing, list):
        if preset_id not in existing:
            existing.append(preset_id)
            existing.sort()
        return
    if existing != preset_id:
        preset_index[normalized_name] = sorted([existing, preset_id])


def extract_text() -> str:
    result = subprocess.run(
        ["pdftotext", str(SOURCE_PATH), "-"],
        check=True,
        capture_output=True,
        text=True,
    )
    return result.stdout


def parse_entries(text: str) -> list[dict[str, int | str]]:
    lines = [line.strip() for line in text.splitlines() if line.strip()]
    entries: list[dict[str, int | str]] = []
    i = 0

    while i < len(lines):
        if lines[i] == "ID" and i + 1 < len(lines) and lines[i + 1] == "Preset Name":
            i += 2
            ids: list[int] = []
            while i < len(lines) and re.fullmatch(r"\d+", lines[i]):
                ids.append(int(lines[i]))
                i += 1

            names: list[str] = []
            while i < len(lines) and lines[i] not in {"KDFX", "ID"}:
                # Skip page markers like 10-3 and 10-4 if they appear in-stream.
                if not re.fullmatch(r"\d+-\d+", lines[i]):
                    names.append(lines[i])
                i += 1

            if i >= len(lines) or lines[i] != "KDFX" or i + 1 >= len(lines) or lines[i + 1] != "Alg":
                continue

            i += 2
            algorithms: list[int] = []
            while i < len(lines) and re.fullmatch(r"\d+", lines[i]):
                algorithms.append(int(lines[i]))
                i += 1

            count = min(len(ids), len(names), len(algorithms))
            for idx in range(count):
                entries.append(
                    {
                        "id": ids[idx],
                        "name": names[idx],
                        "normalizedName": normalize_name(names[idx]),
                        "algorithmId": algorithms[idx],
                    }
                )
            continue

        i += 1

    return entries


def main() -> None:
    dataset = json.loads(LOOKUP_PATH.read_text(encoding="utf-8"))
    presets_by_id = dataset.setdefault("presetsById", {})
    preset_index = dataset.setdefault("presetIdsByNormalizedName", {})
    algorithms_by_id = dataset.get("algorithmsById", {})

    parsed_presets = parse_entries(extract_text())
    for preset in parsed_presets:
        algorithm = algorithms_by_id.get(str(preset["algorithmId"]))
        presets_by_id[str(preset["id"])] = {
            "id": preset["id"],
            "name": preset["name"],
            "normalizedName": preset["normalizedName"],
            "algorithmId": preset["algorithmId"],
            "algorithmName": algorithm.get("name", "") if isinstance(algorithm, dict) else "",
            "sourceLabel": "KDFX v1",
            "v1": True,
            "v2": False,
        }
        add_preset_index(preset_index, str(preset["normalizedName"]), int(preset["id"]))

    meta = dataset.setdefault("meta", {})
    preset_source_files = list(meta.get("presetSourceFiles", []))
    source_file = SOURCE_PATH.name
    if source_file not in preset_source_files:
        preset_source_files.append(source_file)
    meta["presetSourceFiles"] = preset_source_files
    meta["presetCount"] = len(presets_by_id)

    dataset["presetsById"] = dict(sorted(presets_by_id.items(), key=lambda item: int(item[0])))
    dataset["presetIdsByNormalizedName"] = dict(sorted(preset_index.items()))

    LOOKUP_PATH.write_text(json.dumps(dataset, indent=2) + "\n", encoding="utf-8")

    duplicate_names = {
        name: ids
        for name, ids in dataset["presetIdsByNormalizedName"].items()
        if isinstance(ids, list) and len(ids) > 1
    }

    print(f"Wrote {LOOKUP_PATH}")
    print(f"Imported Base ROM presets: {len(parsed_presets)}")
    print(f"Total presets: {meta['presetCount']}")
    print(f"Duplicate normalized names: {len(duplicate_names)}")


if __name__ == "__main__":
    main()
